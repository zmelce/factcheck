...
def extract_chapeau_from_html(html: str) -> str:
    soup = BeautifulSoup(html, "lxml")

    el = soup.select_one("div.defacto-fact-check-chapeau")
    if el:
...
def merge_chapeau(chapeau: str, body: str) -> str:
    chapeau = clean_lines(chapeau or "")
    ...
    if chapeau.lower() in head:
        return body

    return clean_lines(f"{chapeau}\n\n{body}")
...
def extract_with_readability(html: str) -> Tuple[str, str]:
    ...
    chapeau = extract_chapeau_from_html(html)
    body = merge_chapeau(chapeau, body)
...
def extract_with_fallback(html: str) -> Tuple[str, str]:
    ...
    if not container:
        ...
        chapeau = extract_chapeau_from_html(html)
        return title, merge_chapeau(chapeau, text)
